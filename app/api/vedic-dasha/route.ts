...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Dasha API Error:', errorText);
      return NextResponse.json({ error: 'API request failed', details: errorText }, { status: response.status });
    }

    const data = await response.json();
    console.log('Dasha API Response:', JSON.stringify(data).substring(0, 500));
    
    // The API returns the data directly, not wrapped in output
    return NextResponse.json({ success: true, data: data });

  } catch (error: any) {
    console.error('Dasha API Exception:', error);
