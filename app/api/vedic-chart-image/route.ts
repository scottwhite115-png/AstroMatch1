...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get planetary positions to build our own chart
    ...
    const apiUrl = `https://api.prokerala.com/v2/astrology/planet-position?${params.toString()}`
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error("Chart Data API Error (text):", text)
        return NextResponse.json(
          { error: `Failed to fetch chart data: ${response.status} ${response.statusText}` },
          { status: response.status },
        )
      }
      return NextResponse.json(
        { error: errorData.message || errorData.errors?.[0]?.detail || "Failed to fetch chart data from Prokerala" },
        { status: response.status },
      )
    }
    ...
    // Return planetary positions for custom chart rendering
    return NextResponse.json({
      success: true,
      data: data,
    })
