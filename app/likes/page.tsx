...
import { buildConnectionBoxFromAstro, getRank, deriveElement, deriveTrine } from "@/lib/compat/engine"
...
export default function LikesPage() {
  ...
  useEffect(() => {
    ...
    for (const profile of likesProfiles) {
      try {
        ...
        boxes[profile.id] = buildConnectionBoxFromAstro(userAstro, profileAstro)
  ...
  return (
    <div className={`${theme === "light" ? "bg-white" : "bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900"} min-h-screen relative overflow-x-hidden touch-pan-y pb-24`} style={{ overscrollBehavior: 'contain' }}>
      <div className="relative z-10">
        ...
        {selectedProfile && (
          <div className={`fixed inset-0 ${theme === "light" ? "bg-white" : "bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900"} z-50 overflow-y-auto`}>
            <div className="min-h-screen pb-24">
              {/* Close Button */}
              <button
                onClick={handleCloseProfile}
                className="fixed top-4 right-4 z-50 w-10 h-10 rounded-full bg-slate-800/80 backdrop-blur-sm flex items-center justify-center text-white hover:bg-slate-700 transition-all"
              >
                <X className="w-6 h-6" />
              </button>
            ...
            <div className="space-y-4">
              {filteredProfiles.map((profile) => (
                <div
                  ...
                  className={`message-profile-card astro-highlight-card ${theme === "light" ? "bg-white border border-black/20 shadow-lg" : "bg-slate-800/40 border border-indigo-500/20 shadow-lg shadow-indigo-950/30"} backdrop-blur-md rounded-2xl overflow-hidden cursor-pointer transition-all`}
                >
