...
export default function ChatPage() {
  ...
  const convertSimpleToConnectionBoxData = (
    ...
    const { getConnectionBlurb, getMatchLabel, deriveArchetype, deriveWesternEase } = require('@/lib/connectionUi');
    ...
    const labelToRankKey: Record<string, any> = {
      "Triple Harmony": "perfect",
      "Soulmate Match": "perfect", // Legacy support
      "Twin Flame Match": "excellent",
      "Excellent Match": "excellent",
      "Six Harmonies": "excellent",
      "Secret Friends Match": "excellent", // Legacy support
      ...
      "Magnetic Opposites": "fair", // Legacy support
      "Neutral": "fair",
      "Neutral Match": "fair", // Legacy support
      "Six Harms": "challenging",
      "Punishment": "challenging",
      "Breakpoint": "challenging",
      "Challenging": "challenging",
      "Challenging Match": "challenging", // Legacy support
    ...
    const labelToTier = (label: string): string => {
      if (label === "SOULMATE" || label === "SOULMATE MATCH" || label === "Soulmate Match" || label === "Triple Harmony") return "Triple Harmony";
      ...
      if (label === "Six Harmonies" || label === "Secret Friends Match" || label === "SECRET FRIENDS") return "Six Harmonies";
      if (label === "Favourable Match") return "Favourable";
      if (label === "Good Friends" || label === "Good Friends Match") return "Favourable";
      if (label === "Same Sign" || label === "SAME SIGN" || label === "SAME SIGN MATCH") return "Same Sign";
      if (label === "OPPOSITES_ATTRACT" || label === "OPPOSITES ATTRACT" || label === "Opposites Attract" || label === "Six Conflicts" || label === "Magnetic Opposites") return "Six Conflicts";
      if (label === "NEUTRAL" || label === "NEUTRAL MATCH" || label === "Neutral Match" || label === "Neutral") return "Neutral";
      if (label === "Six Harms" || label === "Punishment" || label === "Breakpoint" || label === "Challenging" || label === "Challenging Match") {
        if (label === "Six Harms") return "Six Harms";
        if (label === "Punishment") return "Punishment";
        if (label === "Breakpoint") return "Breakpoint";
        return "Six Harms"; // Default for Challenging
      }
    ...
    const labelToEmoji: Record<string, string> = {
      "SOULMATE": "✨", "SOULMATE MATCH": "✨", "Soulmate Match": "✨", "Triple Harmony": "✨",
      "TWIN FLAME": "🔥", "TWIN FLAME MATCH": "🔥", "Twin Flame Match": "🔥",
      "HARMONIOUS": "✨", "HARMONIOUS MATCH": "✨", "Excellent Match": "✨",
      "Six Harmonies": "💕", "Secret Friends Match": "💕", // Legacy support
      "Favourable Match": "✨", "Good Friends": "✨", "Good Friends Match": "✨",
      "Same Sign": "🪞", "SAME SIGN": "🪞", "SAME SIGN MATCH": "🪞",
      "OPPOSITES_ATTRACT": "⚡", "OPPOSITES ATTRACT": "⚡", "Opposites Attract": "⚡", "Six Conflicts": "⚡", "Magnetic Opposites": "⚡", // Legacy support
      "NEUTRAL": "✨", "NEUTRAL MATCH": "✨", "Neutral Match": "✨", "Neutral": "✨",
      "Six Harms": "⚠️", "Punishment": "⚖️", "Breakpoint": "💥", "Challenging": "⚠️", "Challenging Match": "⚠️", // Legacy support
      "DIFFICULT": "💔", "DIFFICULT MATCH": "💔", "Difficult Match": "💔",
    };
    
    const labelToColor: Record<string, string> = {
      "SOULMATE": "rgb(212, 175, 55)", "SOULMATE MATCH": "rgb(212, 175, 55)", "Soulmate Match": "rgb(212, 175, 55)", "Triple Harmony": "rgb(212, 175, 55)",
      ...
      "Six Harmonies": "rgb(168, 85, 247)", "Secret Friends Match": "rgb(168, 85, 247)", // Legacy support
      "Favourable Match": "rgb(219, 39, 119)", "Good Friends": "rgb(34, 139, 34)", "Good Friends Match": "rgb(34, 139, 34)",
      "Same Sign": "rgb(6, 182, 212)", "SAME SIGN": "rgb(6, 182, 212)", "SAME SIGN MATCH": "rgb(6, 182, 212)",
      "OPPOSITES_ATTRACT": "rgb(239, 68, 68)", "OPPOSITES ATTRACT": "rgb(239, 68, 68)", "Opposites Attract": "rgb(239, 68, 68)", "Six Conflicts": "rgb(249, 115, 22)", "Magnetic Opposites": "rgb(249, 115, 22)", // Legacy support
      "NEUTRAL": "rgb(34, 139, 34)", "NEUTRAL MATCH": "rgb(34, 139, 34)", "Neutral Match": "rgb(34, 139, 34)", "Neutral": "rgb(34, 139, 34)",
      "Six Harms": "rgb(244, 63, 94)", "Punishment": "rgb(239, 68, 68)", "Breakpoint": "rgb(239, 68, 68)", "Challenging": "rgb(244, 63, 94)", "Challenging Match": "rgb(244, 63, 94)", // Legacy support
    ...
    const loadChatData = async () => {
      ...
      try {
        ...
        if (match) {
          ...
          // No match - try to create instant match if receiver allows instant messages
          console.log('[Chat] No match found, attempting to create instant match...')
          try {
            const res = await fetch('/api/matches/create-instant', {
              method: 'POST',
            ...
            if (res.ok && data.matchId) {
              match = { id: data.matchId, matched_at: new Date().toISOString() }
              setMatchId(match.id)
              ...
              console.log('[Chat] ✅ Instant match created, ready to message')
            } else {
              console.error('[Chat] ❌ Could not create instant match:', data.error)
              setLoadError(data.error || 'Could not start conversation. Please try again.')
            }
          } catch (err) {
            console.error('[Chat] Error creating instant match:', err)
            setLoadError('Failed to start conversation. Please try again.')
          }
        }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Use API route for block enforcement (server-side check)
  ...
  const handleEnableNotifications = async () => {
    setShowMenu(false)
    try {
      const { requestNotificationPermission, saveNotificationPreferences, sendTestNotification } = await import("@/lib/utils/notifications")
      const granted = await requestNotificationPermission()
      if (granted) {
        saveNotificationPreferences({ messages: true })
        sendTestNotification("Notifications enabled", "You'll receive alerts for new messages.")
        alert("Notifications enabled! You'll receive app badges and alerts for new messages.")
      } else {
        alert(
          "To enable notifications, please allow notifications when prompted, or go to your browser/phone Settings > Notifications and turn on notifications for this app.",
        )
      }
    } catch (err) {
      console.error("[Chat] Error enabling notifications:", err)
      alert(
        "To enable notifications, please go to your phone's Settings > Notifications and turn on notifications for this app.",
      )
    }
  }
          ...
          {conversation && (
            <MatchProfileCard
              profile={{
                ...
                photos: (() => {
                  const raw = otherUserProfile?.photos
                  if (Array.isArray(raw)) {
                    const filtered = raw.filter((url: any) => url != null && String(url).trim() !== '')
                    return filtered.length > 0 ? filtered : (conversation?.userPhoto ? [conversation.userPhoto] : ["/placeholder.svg"])
                  }
                  if (otherUserProfile?.photos?.[0]) return [otherUserProfile.photos[0]]
                  return conversation?.userPhoto ? [conversation.userPhoto] : ["/placeholder.svg"]
                })(),
