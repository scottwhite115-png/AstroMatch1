...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🌙 Calling Free Astrology API (dasha periods) with:', {
      year: parseInt(year),
    ...
    console.log('📤 Dasha request body:', JSON.stringify(requestBody, null, 2));
    ...
    console.log('📥 Dasha API Response status:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ Dasha API Error response:', errorText);
      ...
      return NextResponse.json(
        { 
          error: 'Failed to fetch dasha periods from Free Astrology API', 
    ...
    console.log('✅ Success! Dasha periods data received');
    
    return NextResponse.json({ success: true, data });

  } catch (error: any) {
    console.error('💥 Free Astrology Dasha API error:', error);
    return NextResponse.json(
