  ...
  return (
    <div className="w-full">
      {/* Match Box - Signs, Match Label Pill, and Blurb - border removed */}
        ...
        <div className="mb-4 mt-2 flex flex-col items-center justify-center w-full" style={{ position: 'relative', zIndex: 10000 }}>
          ...
          {showMatchLabelDropdown && (
            ...
            >
                    ...
                    >
                    {/* SWAPPED: When showProfile=true (Match Engine dropdown), show Profile Info instead of Compatibility */}
                    {/* CRITICAL: When externalShowProfile === true, ONLY show profile info, NEVER show compatibility */}
                    {/* Remove all compatibility sections from dropdown when showProfile=true */}
                    {(() => {
                      // Debug logging
                      const isProfileMode = externalShowProfile === true;
                      if (showMatchLabelDropdown) {
                        console.log('[ConnectionBox Dropdown] Rendering dropdown:', {
                          externalShowProfile,
                          isProfileMode,
                          aboutPartnerText: aboutPartnerText ? `${aboutPartnerText.substring(0, 50)}...` : 'EMPTY',
                          aboutPartnerTextLength: aboutPartnerText?.length || 0
                        });
                      }
                      return isProfileMode;
                    })() ? (
                      /* PROFILE INFO ONLY - No compatibility sections here */
                      <>
                        {/* About Me - Always show section when showProfile=true */}
                        <div style={{ marginBottom: '1.5rem', marginTop: '0' }}>
                          ...
                          {aboutPartnerText && aboutPartnerText.trim() ? (
                            <p 
                            ...
                            <p className={`text-lg ${theme === "light" ? "text-slate-500" : "text-slate-400"}`}>
                        ...
                        {/* Show message if no profile data available but showProfile=true */}
                      ...
                      /* When externalShowProfile is NOT true, but we still want to show About me if available */
                      /* Show About me section even if externalShowProfile is not explicitly true */
                      <>
                        {/* About Me - Show if we have the text, regardless of showProfile prop */}
                        {aboutPartnerText && aboutPartnerText.trim() ? (
                          <div style={{ marginBottom: '1.5rem', marginTop: '0' }}>
                            <h4 
                              className="text-lg font-semibold mb-0.5"
                          ...
                          /* If no About me text, show compatibility sections (for backwards compatibility) */
                          <>
        ...
        {/* SWAPPED: When showProfile=false (View tab), show Compatibility instead of Profile Info */}
        {!showAbout && (connectionOverviewText || westernCompatibilityDescription) && (
          <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: showMatchLabelDropdown ? 'none' : `1px solid ${theme === "light" ? "#e2e8f0" : "#334155"}` }}>
            {/* Chinese Zodiac Compatibility Section */}
            {connectionOverviewText && (
              <div className="mb-4 text-center" style={{ paddingLeft: '1rem', paddingRight: '1rem' }}>
                {/* Chinese Signs Display */}
                {chineseAnimalA && chineseAnimalB && (
                  <div className="flex items-center justify-center gap-1.5 mb-2">
                    {userAChineseIcon && <span className="text-2xl">{userAChineseIcon}</span>}
            ...
            {!connectionOverviewText && !westernCompatibilityDescription && (
              <div className={`text-center py-4 ${theme === "light" ? "text-slate-600" : "text-slate-400"}`} style={{ paddingLeft: '1rem', paddingRight: '1rem' }}>
                <p>No compatibility information available</p>
              </div>
            )}
          </div>
        ...
        {/* Profile Information - Only show when showProfile=true (not in View tab) */}
